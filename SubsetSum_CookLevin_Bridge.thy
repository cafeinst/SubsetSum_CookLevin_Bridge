...
proof -
  ...
  have base: "card (seenL as s k) + card (seenR as s k) ≤ steps as s"
    by (rule steps_lb)
...
proof -
  obtain k where
    k_le: "k ≤ n" and
    base: "card (LHS (e_k as s k) n) + card (RHS (e_k as s k) n)
           ≤ steps as s"
  ...
  also have "… ≤ real (steps as s)"
    using base by simp
  finally show ?thesis .
qed

end  (* locale SubsetSum_Lemma1 *)
